...
### Phase 2: 設定 & API連携（約2日）

**目標**: Claude APIが呼び出せる状態

#### タスク
- [ ] 設定パネルUI（SettingsPanel）の実装
- [ ] APIキーの入力・保存（localStorage暗号化）
  - `encryption.ts` で AES-GCM 暗号化
  - settingsStore にAPIキー・モデル設定を保持
- [ ] Claude APIサービス（claudeService.ts）の実装
  - `@anthropic-ai/sdk` を使用
  - プロンプト設計（ノードのコンテキストを含めた提案依頼）
  - エラーハンドリング（APIキー未設定、レート制限、タイムアウト）
- [ ] ノード選択パネル（NodePanel）の実装
  - 「AIに拡張を依頼」ボタン
  - ノード編集・削除・色変更UI
- [ ] AI提案パネル（AISuggestionPanel）の実装
  - 提案一覧表示（チェックボックス選択）
  - 「追加」「再生成」ボタン
  - ローディング表示
- [ ] 選択した提案を新ノードとして追加するロジック
...
### Phase 3: Googleドライブ連携（約2日）

**目標**: データをGoogleドライブに保存・読み込みできる状態

#### タスク
- [ ] Google Cloud Project 設定手順書の作成（OAuth 2.0クライアントID取得）
- [ ] Google API クライアント（gapi）のセットアップ
- [ ] useGoogleAuth フックの実装
  - サインイン・サインアウト
  - 認証状態の保持
- [ ] googleDriveService.ts の実装
  - ファイル一覧取得（IdeaMapフォルダ内）
  - ファイル作成・更新（JSONアップロード）
  - ファイル読み込み
  - ファイル削除
- [ ] マップ管理UI
  - マップ一覧画面（既存マップの読み込み）
...
- [ ] useAutoSave フックの実装
  ...
  - デバウンス（3秒後に自動保存）
  - 保存状態の表示（「保存中...」「保存済み」）
- [ ] オフライン時のフォールバック（localStorageに一時保存）

**完了条件**: Googleドライブにマップが保存・読み込みできる

---

### Phase 4: UX改善 & 仕上げ（約2日）

**目標**: 実用的なアプリとしての完成度

#### タスク
- [ ] キーボードショートカット（Delete削除、Ctrl+Z 元に戻す）
- [ ] Undo/Redo機能（Zustandのミドルウェアで履歴管理）
- [ ] ノードの自動整列ボタン（dagre レイアウトアルゴリズム）
- [ ] ミニマップの実装（React Flowのビルトイン）
- [ ] ダーク/ライトテーマ切替
- [ ] スマホタッチ操作の最適化
  - ロングプレスでコンテキストメニュー
  - ピンチ操作の微調整
- [ ] エラー表示（トースト通知）
- [ ] AIノードの視覚的区別（アイコン・アニメーション）
- [ ] パフォーマンス最適化（React.memo、useMemo）
- [ ] README.md の作成（セットアップ手順、Google OAuth設定方法）
