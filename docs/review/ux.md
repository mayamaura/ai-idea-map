...
> **2026-08-05 追記（Phase 31）**: 以下4つのチェックリストは Playwright による自動確認を実施済み。結果と、確認によって判明した期待値の誤り・仕様変更は `docs/implementation-plan.md` の各フェーズ「動作確認」節に記録した。本表の「期待値」列のうち訂正が必要だったものは打ち消し線で更新してある。
...
| 5 | @メンション後のクイック質問チップが消える | ~~消える~~ **消えない**（チップは `selectedNode` のみに依存）| Phase 31 で確認。期待値が誤り |
...
| 9 | 「クリア」ボタンを押す | ~~確認なし~~ **確認ダイアログが出る**（Phase 30 で追加）| Phase 31 で確認。期待値が旧仕様 |
| 10 | Ctrl+Shift+C でパネルトグル | 開閉できる | ショートカット実装確認済み |
| 11 | パネル外クリック（PC）で閉じないこと | キャンバス操作が継続できる | sm:hidden マスクで対応 |
| 12 | パネル外タップ（スマホ）で閉じる | **閉じない**（パネルが `w-full h-full` でマスクを覆い到達不可）| Phase 31 で確認。要対応 |
...
| 6 | ツールバー「発表」ボタンで PresentationOrderPanel が開く | 左ボタン＝直接発表開始／右ドロップダウン「発表順を編集してから発表」でパネルが開く | Phase 24 で導線変更。Phase 31 で確認 |
...
| # | 確認項目 | 期待値 | 備考 |
|---|----------|--------|------|
| 1 | ノードをロングプレス（500ms）でコンテキストメニューが開く | 振動フィードバック＋下部シートメニュー | **要実機確認（エミュレータで再現不可）**。実際の起動経路はブラウザの `contextmenu` |
