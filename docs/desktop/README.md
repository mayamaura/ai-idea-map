...
**Phase 40〜53**（`docs/roadmap.md` の v1.0〜v2.0 提案の実施分）も同様の理由で上表・mermaid 図には含めていません。いずれもデスクトップ固有の設計変更を伴わない機能追加（AI機能拡張・成果物生成・PWA化・非同期共同編集など、`packages/core`/`packages/ui` 中心の変更）のため、本 README への設計判断の追記は発生していません。詳細は `docs/implementation-plan.md` を参照。
