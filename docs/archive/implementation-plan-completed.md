...
このファイルは [docs/implementation-plan.md](../implementation-plan.md) から分離した完了済みフェーズ（見出しが `✅ 完了（日付）`）の記録である。新フェーズの起票・進行中フェーズの管理は `docs/implementation-plan.md` で行う。
...
### Phase 11: デバイス間連携 & スタートアップ体験改善（約4日） ✅ 完了（2026-06-01）
...
### Phase 12: ノードUX細部改善 & グループ化（約3日） ✅ 完了（2026-06-04）
...
### Phase 54: ドキュメント再構成（約1日）✅ 完了（2026-08-19）

**目標**: コードより先にドキュメントを整理し、以降のフェーズ管理を正常化する。

#### タスク
- [x]✅ `docs/implementation-plan.md`: 完了済み（✅）フェーズを `docs/archive/implementation-plan-completed.md`（新規）へ分離し、本体は未完了フェーズ＋全フェーズのサマリ表のみにする
- [x]✅ `docs/implementation-plan.md`: `## 1-B. デスクトップアプリ版（Phase 32〜38）` 節を解消し、全フェーズを `## 1. 実装フェーズ` 直下に統合する（CLAUDE.md のルールと一致させる）
- [x]✅ `docs/implementation-plan.md`: フェーズ見出しのステータス表記を CLAUDE.md 定義の3種（なし／`🔨 実装済み（確認中）`／`✅ 完了（日付）`）に一括統一する（「実機確認待ち」等の亜種4件を統一）
- [x]✅ `docs/implementation-plan.md`: Phase 11・Phase 12 の見出しに欠落していた完了マーカーを補う（本文の完了記録から Phase 11 = 2026-06-01、Phase 12 = 2026-06-04 と判定）
- [x]✅ `docs/implementation-plan.md`: `## 4. スケジュール概要`・`## 5. リスクと対策` に「Phase 39 以降は本表に含まれない。各フェーズ節を参照」と注記し、集計対象外であることを明示する
- [x]✅ `docs/requirements.md`: 見出し番号の重複を修正する（旧 2.4.7 重複→2.4.8、旧 2.4.1/2.4.2→2.5.1/2.5.2、旧 2.5 設定機能→2.6。他ドキュメントからの旧番号参照はなしと確認）
- [x]✅ `docs/design.md`: §6 型定義表に `MapFile.presentationNodeIds` と `AISuggestion.parentNodeId` を追記し、存在しない `SuggestionType` を削除する
- [x]✅ `docs/design.md`: §4.2 uiStore 状態表に `currentMapId`・`dragOverGroupId`・`isShortcutsModalOpen` を追記し、§15/§16 の見出し番号の順序入れ替わりを修正する（§16.3 への参照も追従）
- [x]✅ `docs/roadmap.md`: ヘッダーの矛盾を解消し、「やらないと決めたこと」（§8）を残して提案書部分をアーカイブ化。完了済みマイルストーンは `implementation-plan.md` への参照のみにする
- [x]✅ `docs/desktop/README.md`: 最終更新日を 2026-08-19 に更新し、ロードマップ節（§4）に Phase 40〜53 の状況を注記する

**完了条件**: ドキュメント更新のみのため、ビルド影響なし・リンク切れなしであること。→ 2026-08-19 達成。docs 配下全 Markdown の相対リンクチェックで切れ 0 件。コード変更なしのためビルド影響なし。
