...
### 11.8 放射状自動整列（`applyRadialLayout`）

ルート（入力エッジなしのトップレベルノード）を中心 `(0,0)` に置き、深さごとの同心リング上に配置する。座標は中心基準で計算し、最後にノードサイズの半分を引いて左上座標に直す（グループノードは `style.width/height` の実サイズを使う）。

重なりを構造的に防ぐため、以下の3点を守る:

| 項目 | 決め方 | 理由 |
|---|---|---|
| 角度配分 | サブツリーの**葉の数**に比例したくさび（角度範囲）を割り当てる | ノード総数だと、縦に深いだけのサブツリーが横幅を無駄に占有する |
| 配置基準 | 親からの相対ではなく**中心からの絶対半径** | くさびが交わらない限り別サブツリー同士が絶対に重ならない |
| 半径 | 第1リング = `max(220, 葉の総数 × (最大ノード幅 + 60) / 2π)`、以降 `+ max(190, hypot(最大幅, 最大高) + 60)` | 第1リングの式は「1葉あたりの弧長 ≧ ノード幅＋すき間」を保証する。リング間隔がノードの対角線を超えていれば、中心からの距離差 ≧ リング間隔 なので角度に関係なく重ならない |

孤立ノード（ルートから到達できないノード）は最外リングの外側に横並びで置く。自己チェックは `pnpm check:radial`（`packages/core/verify-radial-layout.mts`）で、木の幅・深さを変えた6パターン＋孤立ノードありで全ノードの矩形が重ならないことを検証する。
