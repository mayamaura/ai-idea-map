...
### Phase 5: 右クリックメニュー & インタラクション全面改善（約3日）
...
- 設定でマップ全体の矢印表示ON/OFFをトグル可能

#### タスク

**右クリックコンテキストメニュー** (`src/components/canvas/ContextMenu.tsx`)
- [ ] キャンバス右クリック → 「アイデアを作成」「ここに貼り付け」
- [ ] ノード右クリック → 「アイデアを作成（接続）」「ノードを削除」「AI拡張」「コピー」「カテゴリを変更」「詳細を開く」
- [ ] エッジ右クリック → 「エッジを削除」「ラベルを追加」「向きを変更」「双方向にする」
- [ ] Escキーでメニューを閉じる、メニュー外クリックで閉じる
- [ ] React Portalで `<body>` 直下にレンダリング（z-index問題を回避）

**ノード削除UX改善**
- [ ] DELキー以外の削除手段：右クリックメニュー、バックスペースキー、ツールバーボタン
- [ ] 複数選択状態での一括削除（DEL / Backspace）
- [ ] 削除確認モーダル（子ノードがある場合のみ表示）
- [ ] 「接続エッジのみ削除」オプション（ノード自体は残す）

**ハンドル改善：全方向接続対応**
- [ ] IdeaNode の Handle を `type="source"` と `type="target"` の両方を全4方向に配置 → 任意方向からの接続を可能にする
- [ ] ホバー時のみハンドルを表示（通常時は非表示にしてスッキリ見せる）
- [ ] 接続開始時のハイライト演出

**ノード作成UX改善**
- [ ] 選択ノードから `Tab` キーで接続された新ノードを作成
- [ ] 選択ノードの右クリックメニュー「アイデアを作成（接続）」で隣に新ノードを追加
- [ ] ツールバーの「追加」ボタンを「選択ノードの子として追加」「独立して追加」に分岐

**マルチセレクト & コピー・ペースト**
- [ ] Shift+クリックで複数選択（既存のReact Flow機能を有効化）
- [ ] Ctrl+C / Ctrl+V でノードをコピー・ペースト（位置オフセット+20px）
- [ ] 選択中のノード群を一括移動・削除

**有向エッジの実装**
- [ ] `mapStore.onConnect` に `markerEnd: { type: MarkerType.ArrowClosed }` を追加
- [ ] `IdeaEdge.tsx` カスタムエッジコンポーネント（クリックで選択、右クリックでメニュー）
- [ ] エッジの向き変更ロジック（source/target を入れ替え）
- [ ] エッジを「双方向」にする場合は `markerStart` と `markerEnd` 両方に矢印
