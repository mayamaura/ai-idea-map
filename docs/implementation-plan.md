...
### Phase 14: AIチャット & マップ対話（約3日）

**目標**: AIを「マップ全体と対話できる思考パートナー」に昇格させる

#### 設計方針
- 現在のAI機能（ノード個別拡張、マップ分析）に加え、自由形式のチャットでマップについて議論できる
- チャット履歴はセッション内のみ保持（Drive保存はオプション）
- AIはマップの全ノード・エッジ・カテゴリ情報をコンテキストとして保持した上で回答

#### タスク

**AIチャットパネル** (`src/components/panels/AIChatPanel.tsx`)
- [ ] ヘッダーに「AIチャット」ボタンを追加（MapAnalysisPanelの隣）
- [ ] サイドパネル形式のチャットUI（メッセージ履歴 + 入力フォーム）
- [ ] `claudeService.ts` に `chatWithMap(messages, mapContext)` 関数を追加
  - 初回メッセージにマップ全体のコンテキストを埋め込み
  - 以降は会話履歴を引き継いで連続対話
- [ ] AIの回答にマップ操作の提案が含まれる場合、ワンクリックで実行できるアクションボタンを表示
  - 例：「ノードを追加しますか？」→「追加」ボタン
  - 例：「これらのノードを接続しますか？」→「接続」ボタン

**コンテキスト認識機能**
- [ ] チャット中に `@ノード名` でノードを参照できる（オートコンプリート付き）
- [ ] 選択中のノードがある場合は「このノードについて」のクイック質問チップを表示
  - 「もっと深掘りして」「反論を考えて」「具体的なアクションに落とし込んで」など
