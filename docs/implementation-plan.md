...
- [ ] `GroupNode` コンポーネントを新規作成（ラベル付き半透明ボックス、リサイズハンドル付き）
- [ ] `mapStore` に `addGroupNode` / `ungroupNodes` アクションを追加
- [ ] キャンバス右クリック → 「グループを作成」メニュー項目追加
- [ ] 複数選択ノードを右クリック → 「グループ化」でグループノードを生成し選択ノードを子に設定

##### グループ操作UX

- [ ] グループヘッダーをダブルクリックでラベル編集
- [ ] グループを移動すると子ノードも追従（React Flow の `parentId` 機能を利用）
- [ ] グループを削除 → 「グループのみ削除（子ノードは残す）」「グループと子ノードを削除」の選択ダイアログ
